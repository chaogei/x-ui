...
func (a *XUIController) index(c *gin.Context) {
	html(c, "status", I18n(c, "menu_system_status"), nil)
}

// inbounds 渲染入站列表页。
//
// initialCredentials 驱动页面顶部的安全告警：为 true 说明管理员仍在使用
// 首次启动自动生成的随机口令，尚未自行改密。历史代码把这个提示写死为
// `v-if="false"` 永久隐藏，等于告警形同虚设。
func (a *XUIController) inbounds(c *gin.Context) {
	html(c, "inbounds", I18n(c, "menu_inbound_list"), gin.H{
		"initialCredentials": a.userService.UsingInitialCredentials(),
	})
}

func (a *XUIController) setting(c *gin.Context) {
	html(c, "setting", I18n(c, "menu_panel_setting"), nil)
}
